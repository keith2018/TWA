...
parser.add_argument("--exec", metavar="<interpreter>", default="../bin/twa cmd")
...
def binaryToFloat(num, t):
    try:
        if t == "f32":
            return struct.unpack('!f', struct.pack('!L', int(float(num))))[0]
        elif t == "f64":
            return struct.unpack('!d', struct.pack('!Q', int(float(num))))[0]
        else:
...
class Wasm3():
    ...
    def run(self):
        ...
        try:
            self._read_until("twa> ")
        except Exception as e:
            print(f"twa: Could not start: {e}")

    def restart(self):
        print(f"twa: Restarting")
        for i in range(10):
            try:
                self.run()
                try:
